...
#region Public methods
        ...
        def update(self, delta_time):
            ...
            if (FpsSettings.USE_DDA_RENDERING):
                self._prepare_cells(raycast_hits)
        ...
        def draw(self, screen, st):
            ...
            if (FpsSettings.USE_DDA_RENDERING):
                ...
                self._draw_matrix_prototype_and_objects(screen, offset, st)
...
#region Private methods
        ...
        def _prepare_cells(self, raycast_hits):
            for ray_index, hits in enumerate(raycast_hits):
                for hit in hits:
        ...
        def _draw_objects(self, screen, offset, st):
            ...
            self.objects_to_render = sorted(self.objects_to_render, key=lambda p: p.near_depth, reverse=True)

            for projection_result in self.objects_to_render:
                self._draw_object_item(screen, offset, st, projection_result)


        def _draw_object_item(self, screen, offset, st, projection_result):
        ...
        def _draw_matrix_prototype_and_objects(self, screen, offset, st):
            """
            ...
            combined.extend((p.near_depth, "object", p) for p in self.objects_to_render)

            combined.sort(key=lambda item: item[0], reverse=True)

            for depth, kind, payload in combined:
                if kind == "object":
                    self._draw_object_item(screen, offset, st, payload)
                    continue
                ...
                try:
                    ...
                    with open("C:/Users/Mikkel/AppData/Local/Temp/claude/c--Users-Mikkel-Documents-Repositories-RenPy-RenpyFPS/fa4cd9a0-d30a-4ceb-a0ac-5274e138f2b8/scratchpad/split_debug.txt", "a") as f:
                        f.write("depth=%r cell=%r side=%r edge_override=%r plane=%r -> %r\n" % (
                            depth, cell.coord if cell else None, side, edge_override, plane_cell.coord if plane_cell else None, e))
        ...
        def _collect_matrix_draw_items(self):
            ...
            raycast_results = self.raycast_hits
            ...
            with open("C:/Users/Mikkel/AppData/Local/Temp/claude/c--Users-Mikkel-Documents-Repositories-RenPy-RenpyFPS/fa4cd9a0-d30a-4ceb-a0ac-5274e138f2b8/scratchpad/split_debug.txt", "w") as f:
                for target in targets:
                    if target.cell.type in (ECellType.HorizontalDoor, ECellType.VerticalDoor):
                        c = target.cell
                        f.write("door=%r grid_alignment=%r open_direction=%r open_amount=%r offset=%r thickness=%r\n" % (c.coord, c.grid_alignment, c.open_direction, c.open_amount, c.offset, c.thickness))
                f.write("suppressed=%r\n" % (suppressed_faces,))
                for seg in split_segments:
                    cell, side, seg_min, seg_max, crop_x_fraction, piece_label = seg
                    f.write("segment cell=%r side=%r range=[%r,%r] crop_x_fraction=%r piece=%r\n" % (cell.coord, side, seg_min, seg_max, crop_x_fraction, piece_label))
